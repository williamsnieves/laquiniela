...
class Semifinal(models.Model):
	...
	progress_semis = models.CharField(max_length=6,default='')
...
class ViewSemis:
	def getMatchesSemis(self,cod_qnl=None):
		...
		cursor.execute("SELECT * FROM vw_semifinal_qnl WHERE cod_qnl='"+cod_qnl+"' ORDER BY 4")
		position_list = []
		final_list = []
		position_dict = {}
		...
		for i in range(len(position_list)):
			position_dict = dict(cod_qnl=position_list[i][0],codjuego=position_list[i][1],cod_juego=position_list[i][2],equipo=position_list[i][3])
